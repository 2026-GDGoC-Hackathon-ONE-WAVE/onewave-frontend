...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full h-[80px] bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 md:px-20 flex items-center justify-between">
      ...
      {!isOnboarding && (
        <>
          ...
          <div className="flex items-center gap-4">
            <button className="hidden sm:block px-6 py-2.5 text-[15px] font-bold text-gray-600 hover:text-[#222222]">
              로그인
            </button>
            <div className="flex items-center gap-3 pl-4 border-l border-gray-100">
              <img 
                src="https://vinsign.app/resources/avatars/avatar-5.png" 
                className="w-10 h-10 rounded-full border-2 border-white shadow-sm" 
                alt="User" 
              />
              <span className="hidden lg:block text-gray-700 font-bold text-sm">김지우 님</span>
            </div>
          </div>
        </>
      )}
