...
const Experiences = () => {
  ...
  const [experienceList, setExperienceList] = useState([
    {
      ...
      simpleMemo: "면접 분위기가 매우 유연하고 수평적이었음. 사용자 데이터 기반의 의사결정 과정을 깊게 물어보셨음."
    },
    {
      id: 2,
      company: "당근 (당근마켓)",
    ...
    },
    {
      ...
      step: "기술 인터뷰",
      progress: "회고 완료",
      imgUrl: "https://gmcnqdpighpxhzpesqwf.supabase.co/storage/v1/object/public/generated-images/image-2eea60f6-d195-4d92-94f6-767fca02f7b1.jpg",
      simpleMemo: "자료구조와 알고리즘 위주의 질문이 많았음. 특히 대규모 트래픽 처리 경험에 대해 깊게 논의했는데, 논리적으로 설명하여 긍정적인 반응을 얻음."
    }
  ]);
  ...
  const handleLogoClick = () => {
    ...
    hasOnboarded ? navigate('/experiences') : navigate('/');
  };
          ...
          <section className="relative w-full h-[240px] bg-gradient-to-br from-[#1E1B4B] to-[#312E81] rounded-[32px] mb-12 overflow-hidden flex items-center justify-between px-16 shadow-2xl">
            <div className="relative z-10 text-left">
              <h1 className="text-4xl font-bold text-white mb-4 leading-tight">
                아직 정리되지 않은<br />
                <span className="text-[#DFFF00]">{experienceList.filter(e => e.progress !== '회고 완료').length}개의 경험</span>이 기다리고 있어요.
              </h1>
            ...
            <div className="bg-white w-full max-w-[550px] rounded-[32px] p-10 shadow-2xl animate-in fade-in zoom-in duration-200 overflow-y-auto max-h-[90vh]">
              ...
              <div className="space-y-6 text-left">
                <div>
                  ...
                  <input name="company" value={newExp.company} onChange={handleChange} type="text" placeholder="예: 토스" className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-gray-400 mb-2">면접 본 날짜</label>
                    <input name="date" value={newExp.date} onChange={handleChange} type="text" placeholder="2025-02-01" className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-400 mb-2">지원 직무</label>
                    <input name="tag" value={newExp.tag} onChange={handleChange} type="text" placeholder="예: Product Designer" className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-400 mb-2">탈락한 전형</label>
                  <div className="relative">
                    <select name="step" value={newExp.step} onChange={handleChange} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium appearance-none cursor-pointer">
                      <option>서류 전형</option>
                ...
                <div>
                  ...
                  <textarea name="simpleMemo" value={newExp.simpleMemo} onChange={handleChange} rows="3" placeholder="면접 당시 느꼈던 점이나 특징을 간단히 적어주세요." className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium resize-none"></textarea>
                </div>
                <button onClick={handleAddExperience} className="w-full py-5 bg-gray-900 text-white rounded-2xl font-black text-lg hover:bg-black transition-all mt-4 shadow-xl active:scale-[0.98]">경험 저장하고 시작하기</button>
              </div>
            </div>
          </div>
        )}
...
const ExperienceCard = ({ company, date, tag, status, step, progress, imgUrl, isStart, simpleMemo }) => {
  const navigate = useNavigate();
  return (
    <div 
      onClick={() => navigate('/experience-detail', { state: { company, tag, date, step, progress, simpleMemo } })}
    ...
    >
      ...
      <div className="mt-8 pt-6 border-t border-gray-50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={`w-2.5 h-2.5 rounded-full ${progress === '회고 완료' ? 'bg-[#D946EF]' : progress === '회고 중' ? 'bg-orange-400 animate-pulse' : 'bg-gray-300'}`}></div>
          <span className="text-sm font-bold text-gray-700">{progress}</span>
        </div>
        <button className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-colors ${progress === '회고 완료' ? 'bg-[#D946EF] text-white shadow-lg' : isStart ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-900 text-white'}`}>
          {progress === '회고 완료' ? '결과 보기' : isStart ? '회고 시작' : '이어하기'}
        </button>
      </div>
    </div>
  );
...
const NavBtn = ({ icon, label, active }) => (
  <button className={`flex flex-col items-center gap-1 ${active ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-600'} transition-colors text-left`}>
